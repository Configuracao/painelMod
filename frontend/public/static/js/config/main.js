...
document.querySelector('.create-config').onclick = () => {
    if (categories.categories.length <= 0) {
        showToastError('No fueron encontradas categorias.');
        return;
    }
...
document.querySelector('.delete-config').onclick = () => {
    ...
    if (items.length <= 0) {
        showToastError('Seleccione una configuracion.');
        return;
    }
...
list.register(new Observer('append', async item => {
    ...
    try {
        ...
        showToastError('No fue posible crear la Configuracion!');
        list.remove(item.id);
        categories.remove(item.id);
    }
...
list.register(new Observer('update', async item => {
    ...
    try {
        ...
        if (response.status === 200) {
            showToastSuccess('Configuracion atualizada con exito!');
            return;
        }
        ...
        showToastError('No fue posible atualizar Configuracion!');
    } finally {
        renderApp();
    }
}));
...
list.register(new Observer('remove', async id => {
    ...
    try {
        ...
        if (response.status == 204) {
            showToastSuccess('Configuracion removida con exito!');
            return;
        }
        ...
        showToastError('No fue posible remover la Configuracion!');
    } finally {
        renderApp();
    }
}));
